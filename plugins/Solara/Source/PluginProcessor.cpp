...
#include "Presets/SolaraSettings.h"
...
{
    // Prefer the user-chosen folder (set via the editor's first-run prompt
    // or "Locate Presets..." button). Fall back to the legacy convention
    // of a "Solara Presets" folder next to the plugin's own .vst3 bundle.
    {
        juce::File presetDir = SolaraSettings::getPresetFolder();
        ...
        if (! presetDir.isDirectory())
        {
            ...
            for (int depth = 0; depth < 6; ++depth)
            {
                ...
                {
                    presetDir = f.getParentDirectory().getChildFile ("Solara Presets");
                    break;
                }
...
SolaraProcessor::~SolaraProcessor()
{
    delete readyPreset_.exchange (nullptr);
    delete activePreset_;
}
                                     ...
                                     juce::MidiBuffer& midiMessages)
{
    ...
    // Activate a preset that was fully loaded + decoded on the message
    // thread (see requestPresetLoad). This is just a pointer swap — no
    // file I/O or allocation happens here, so it can't stall the audio
    // callback the way loading used to.
    if (auto* newPreset = readyPreset_.exchange (nullptr))
    {
        synth_.allNotesOff (0, false); // immediate stop, no tail — must precede the swap
        synth_.setActivePreset (&newPreset->parser, &newPreset->cache, newPreset->presetIndexInBank);

        // The old preset is no longer referenced by any voice (all were just
        // stopped above), so it's safe to free — deferred to the message
        // thread since delete isn't real-time safe.
        auto* old = activePreset_;
...
{
    const auto* rp = presetDB_.getPreset (globalPresetIndex);
    if (! rp)
        return;

    auto data = std::make_unique<SolaraPendingPreset>();
    if (! data->parser.load (rp->bankFile))
        return;

    const auto* presetEntry = data->parser.getPreset (rp->presetIndexInBank);
    if (! presetEntry)
        return;

    for (uint32_t z = presetEntry->zoneOffset; z < presetEntry->zoneOffset + presetEntry->zoneCount; ++z)
    {
        const auto* zone = data->parser.getZone ((int) z);
        if (! zone) continue;

        const float* rawData = data->parser.getSampleData (*zone);
        if (! rawData) continue;

        auto decoded = SampleCache::decode (*zone, rawData);
        if (decoded)
            data->cache.store (z, std::move (decoded));
    }

    data->presetIndexInBank = rp->presetIndexInBank;

    presetDB_.markLoaded (globalPresetIndex);

    // Publish for the audio thread. If a previous request hasn't been
    // picked up yet (e.g. rapid clicking), it was never activated, so it's
    // safe to delete right here on the message thread.
    if (auto* prev = readyPreset_.exchange (data.release()))
        delete prev;
}

//==============================================================================
void SolaraProcessor::setPresetFolder (const juce::File& folder)
{
    if (folder.isDirectory())
    {
        presetDB_.scanDirectory (folder);
        SolaraSettings::setPresetFolder (folder);
    }
}
...
void SolaraProcessor::getStateInformation (juce::MemoryBlock& destData)
{
    ...
    // Persist the active preset so the project reloads the same sound.
    xml->setAttribute ("solaraPresetIndex", presetDB_.getCurrentGlobalIndex());
...
void SolaraProcessor::setStateInformation (const void* data, int sizeInBytes)
{
    ...
    const int savedIndex = xmlState->getIntAttribute ("solaraPresetIndex", -1);
    apvts_.replaceState (juce::ValueTree::fromXml (*xmlState));

    // Reload the preset that was active when the project was saved.
    // setStateInformation is always called on the message thread, so
    // requestPresetLoad (which does file I/O) is safe to call here.
    if (savedIndex >= 0 && savedIndex < presetDB_.getTotalPresetCount())
        requestPresetLoad (savedIndex);
}
