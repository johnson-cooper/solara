    ...
    : AudioProcessorEditor (&p), processor_ (p), apvts_ (p.getAPVTS())
{
    ...
    addAndMakeVisible (categoryPrevButton_);
    addAndMakeVisible (categoryNextButton_);
    ...
    categoryPrevButton_.onClick = [this] { goToCategory (-1); };
    categoryNextButton_.onClick = [this] { goToCategory (1); };
...
void SolaraEditor::resized()
{
    ...
    auto presetArea = bounds.removeFromTop (120);
    ...
    auto categoryNavArea = presetInner.removeFromTop (22);
    categoryPrevButton_.setBounds (categoryNavArea.removeFromLeft (32));
    categoryNextButton_.setBounds (categoryNavArea.removeFromRight (32));
    categoryNameLabel_.setBounds (categoryNavArea);
...
//==============================================================================
void SolaraEditor::goToCategory (int direction)
{
    auto& db = processor_.getPresetDatabase();
    if (db.getTotalPresetCount() <= 0)
        return;

    const int fromIndex = displayedPresetIndex_ >= 0 ? displayedPresetIndex_ : 0;
    const int newIndex  = db.getNextCategoryIndex (fromIndex, direction);
