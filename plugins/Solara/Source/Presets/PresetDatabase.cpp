...
void PresetDatabase::scanDirectory (const juce::File& dir)
{
    ...
    for (const auto& f : slrFiles)
    {
        ...
        for (int p = 0; p < probe.getPresetCount(); ++p)
        {
            ...
            RuntimePreset rp;
            rp.name               = f.getFileNameWithoutExtension();
            if (probe.getPresetCount() > 1)
                rp.name << " " << (p + 1); // disambiguate multi-preset banks
...
void PresetDatabase::markLoaded (int globalPresetIndex)
{
