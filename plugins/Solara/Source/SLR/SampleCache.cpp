...
SampleCache::decode (const SLR::SampleZone& zone, const float* rawData)
{
    ...
    // A zero/out-of-range root key means the converter didn't set one —
    // default to middle C (60) instead of mistuning playback.
    sample->rootKey    = (zone.rootKey > 0 && zone.rootKey <= 127) ? zone.rootKey : 60;
