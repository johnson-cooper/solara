...
public:
    ...
    void paint (juce::Graphics& g) override
    {
        ...
        // Value arc — normalize the real parameter value (which may have
        // any range, e.g. -24..24) into 0..1 for the arc sweep.
        const float norm = (float) slider_.valueToProportionOfLength (slider_.getValue());
        {
            juce::Path valArc;
    ...
    {
        dragStartY_    = e.y;
        dragStartNorm_ = slider_.valueToProportionOfLength (slider_.getValue());
    }

    void mouseDrag (const juce::MouseEvent& e) override
    {
        const double delta   = (dragStartY_ - e.y) / 200.0;
        const double newNorm = juce::jlimit (0.0, 1.0, dragStartNorm_ + delta);
        slider_.setValue (slider_.proportionOfLengthToValue (newNorm),
...
public:
    ...
    {
        if (attack_ == attack && decay_ == decay && sustain_ == sustain && release_ == release)
            return;
